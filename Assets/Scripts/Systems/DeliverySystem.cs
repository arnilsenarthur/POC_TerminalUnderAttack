    ...
    public class DeliverySystem : SingletonBehaviour<DeliverySystem>
    {
        ...
        private void _OnTick(float deltaTime)
        {
            if (!GameWorld.Instance.IsServerSide)
                return;
            if (deliveryPoint == null || hackerToolItem == null)
                return;
            if (GameWorld.Instance == null || !GameWorld.Instance.IsServerInitialized)
                return;
            ...
            foreach (var entity in allEntities)
            {
                ...
                if (inventoryHolder == null)
                    continue;
                float distance = Vector3.Distance(entity.transform.position, deliveryPoint.position);
                if (distance > radius)
                    continue;
                _Server_ReplaceDataDriveWithHackerTool(inventoryHolder);
            }
        }
        private bool _Server_ReplaceDataDriveWithHackerTool(IInventoryHolder inventoryHolder)
        {
            if (inventoryHolder == null || hackerToolItem == null)
                return false;
            ...
            for (int i = 0; i < inventory.slots.Length; i++)
            {
                if (inventory.slots[i] == null || string.IsNullOrEmpty(inventory.slots[i].item))
                    continue;
                if (inventory.slots[i] is DataDriveItemStack)
                {
                    dataDriveSlot = i;
            ...
            if (feed != null && inventoryHolder is PlayerEntity playerEntity)
            {
                ...
                string player = $"<color={color}>{playerEntity.GamePlayer.Name}</color>";
                string disk = $"<color={diskColor}>{dataDriveItem.targetName}</color>";
